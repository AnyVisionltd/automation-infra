...
def test_ssh(base_config):
    ...
    base_config.SSH.put('/tmp/temp.txt', '/tmp')
    res = base_config.SSH.execute('ls /tmp')
    assert 'temp.txt' in res.split()
    base_config.SSH.execute('rm /tmp/temp.txt')
    res = base_config.SSH.execute('ls /tmp')
...
def test_s3(base_config):
    base_config.Seaweed.create_bucket('test_bucket')
    content = base_config.Seaweed.get_buckets()
    assert b'test_bucket' in content
    base_config.Seaweed.delete_bucket('test_bucket')
    content = base_config.Seaweed.get_buckets()
...
def test_memsql_add_suspect(base_config):
    ...
    res = base_config.Memsql.upsert(query)
    assert res == 1


@hardware_config(hardware={"type": "aio"})
def test_pipeng_features(base_config):
    response = base_config.PipeNg.health_check()
    assert response.status == 1


@hardware_config(hardware={"type": "aio"})
def test_consul_get_services(base_config):
    _, services_dict = base_config.Consul.get_services()
    assert len(services_dict) > 0
    assert 'camera-service' in services_dict
    put_key, put_val = ("test_key", "test_val")
    res = base_config.Consul.put_key(put_key, put_val)
    assert res is True
    val = base_config.Consul.get_key("test_key")
    assert val.decode('utf-8') == put_val


@hardware_config(hardware={"type": "aio"})
def test_kafka_functionality(base_config):
    topics = base_config.Kafka.get_topics()
    assert len(topics.topics) > 0
    success = base_config.Kafka.create_topic('oris_new_topic')
    assert success
    time.sleep(5)
    success = base_config.Kafka.delete_topic('oris_new_topic')
    assert success


@hardware_config(hardware={"type": "aio"})
def test_camera_service(base_config):
    cameras = base_config.CameraService.get_cameras()
    print(cameras)
    assert 1
