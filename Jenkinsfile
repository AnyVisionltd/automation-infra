...
pipeline {
    ...
    stages {
        ...
        stage('Run infra integration tests on docker') {
            stages {
                stage('Spin up VM') {
                    steps {
                        script {
                            try {
                                env.vminfo = SpinUpVM(remote, "ubuntu-compose_v2")
                            } catch (Exception e) {
        ...
        stage('Run infra integration tests on k8s') {
            stages {
                stage('Spin up VM') {
                    steps {
                        script {
                            try {
                                env.vminfo = SpinUpVM(remote, "ubuntu-k8s-1804")
                            } catch (Exception e) {
                                echo " ------ FAILED TO CREATE VM -------"
                                echo e.getMessage()
                                echo " ------ journalctl SYSLOG_IDENTIFIER=HYPERVISOR -n 300 -------"
