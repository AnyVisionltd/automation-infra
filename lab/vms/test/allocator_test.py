...
async def test_allocate_machine_no_gpus(event_loop, mock_libvirt, mock_image_store, mock_nbd_provisioner, mock_cloud_init, mock_dhcp_handler):
    ...
    tested = allocator.Allocator(macs, gpus, manager, "sasha", max_vms=1, paravirt_device="eth0", sol_base_port=5000)
...
async def test_allocate_multiple(event_loop, mock_libvirt, mock_image_store, mock_nbd_provisioner, mock_cloud_init, mock_dhcp_handler):
    ...
    manager = vm_manager.VMManager(event_loop, mock_libvirt, mock_image_store, mock_nbd_provisioner, mock_cloud_init, mock_dhcp_handler)
    tested = allocator.Allocator(macs, gpus, manager, "sasha", max_vms=3, paravirt_device="eth0", sol_base_port=5000)
...
async def test_create_machine_and_restore_machine(event_loop, mock_libvirt, mock_image_store, mock_nbd_provisioner, mock_cloud_init, mock_dhcp_handler):
    ...
    mock_dhcp_handler.allocate_ip = mock.AsyncMock(return_value = "1.1.1.1")
    old_allocator = allocator.Allocator(copy.copy(macs), copy.copy(gpu1), manager, "sasha", max_vms=1, paravirt_device="eth0", sol_base_port=5000)
...
@pytest.mark.asyncio
async def test_concurrent_allocation_and_free_resources(event_loop, mock_libvirt, mock_image_store, mock_nbd_provisioner,
    ...
    with pytest.raises(NotEnoughResourceException):
        await asyncio.gather(tested_allocator.allocate_vm("sasha_image1", memory_gb=1, base_image_size=10, networks=["bridge"], num_cpus=2, num_gpus=1),
                             tested_allocator.allocate_vm("sasha_image1", memory_gb=1, base_image_size=10, networks=["bridge"], num_cpus=2, num_gpus=1),
                             tested_allocator.allocate_vm("sasha_image1", memory_gb=1, base_image_size=10, networks=["bridge"], num_cpus=2, num_gpus=1),
                             tested_allocator.allocate_vm("sasha_image1", memory_gb=1, base_image_size=10, networks=["bridge"], num_cpus=2, num_gpus=1),
                             tested_allocator.allocate_vm("sasha_image1", memory_gb=1, base_image_size=10, networks=["bridge"], num_cpus=2, num_gpus=1),
                             tested_allocator.allocate_vm("sasha_image1", memory_gb=1, base_image_size=10, networks=["bridge"], num_cpus=2, num_gpus=1))

    assert len(tested_allocator.vms) == 1

    await tested_allocator.destroy_vm("sasha-vm-0")
    assert len(tested_allocator.vms) == 0
    assert tested_allocator.gpus_list == gpu1
    assert set(tested_allocator.mac_addresses) == set(macs)
